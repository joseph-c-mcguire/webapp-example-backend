...
class ModelMonitor:
    def __init__(
        self, model: BaseEstimator, X_train: pd.DataFrame, threshold_drift=0.05
    ):
        self.model = model
    ...
    def check_feature_drift(self, X_new):
        ...
        drift = (
            new_means - self.train_feature_means
        ).abs() / self.train_feature_means.abs()
        drift_detected = drift > self.threshold_drift
        drift_report = drift[drift_detected]

        if not drift_report.empty:
            logger.warning(
                "Feature drift detected in features: %s", drift_report.index.tolist()
            )
    ...
    def monitor(self, X_new, y_true, problem_type="classification"):
        ...
        if drift_detected:
            logger.warning("Action required: Significant feature drift detected.")
